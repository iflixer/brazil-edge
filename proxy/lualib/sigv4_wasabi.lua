...
function _M.sign_request()
  ...
  local s3_uri  = ngx.var.s3_uri

  bucket = (bucket or ""):gsub("^/+", ""):gsub("/+$", "")
  key = (key or ""):gsub("^/+", "")
  if bucket == "" or key == "" then
    ngx.log(ngx.ERR, "s3 bucket/key is empty")
    return ngx.exit(500)
  end
  ...
  local canonical_uri = s3_uri
  if not canonical_uri or canonical_uri == "" then
